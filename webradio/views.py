...
def stations(request):
    ...
    stations = Station.objects.all()
    return render(request, 'webradio/stations.html', {'categories': categories, 'stations': stations,})
...
def control_station_next(request):
    ...
    try:
        next_station = Station.objects.filter(id__gt=current_station.id).order_by('id')[0]
    except:
